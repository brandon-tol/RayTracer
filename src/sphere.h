...
namespace btoleda
{
    ...
    {
    public:
        inline sphere(const point3& center, const double radius, const material& mat)
            : scene_object(mat), _m_center(center), _m_radius(radius)
        {}

        virtual bool on_hit(const ray& r, double t_min, double t_max, hit_record& rec) const override;

        static const vec3& random_in_unit_sphere();
    private:
        point3 _m_center;
        double _m_radius;
    };
